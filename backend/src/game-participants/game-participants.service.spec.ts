...
describe('GameParticipantsService', () => {
  ...
  describe('joinGame', () => {
    it('should allow a user to join a game successfully', async () => {
      ...
      expect(mockGameRepository.findOne).toHaveBeenCalledWith({
        where: { gameId: mockGame.gameId },
        relations: ['gameParticipants'],
      });
      expect(mockGameParticipantRepository.create).toHaveBeenCalledWith({
        game: mockGame,
        user: mockUserParticipant,
        status: ParticipationStatus.PENDING,
      });
      expect(mockGameParticipantRepository.save).toHaveBeenCalledWith(
        mockGameParticipant,
      );
    });
    ...
    it('should throw ConflictException if user is already participating', async () => {
      mockGameRepository.findOne.mockResolvedValue(mockGame);
      mockGameParticipantRepository.findOne.mockResolvedValue(
        mockGameParticipant,
  ...
  describe('setStatus', () => {
    it('should update status for an existing participant', async () => {
      ...
      expect(mockGameParticipantRepository.findOne).toHaveBeenCalledWith({
        where: {
          user: { uid: setStatusDto.uid },
          game: { gameId: setStatusDto.gameId },
        },
        relations: ['user', 'game'],
      });
      expect(mockGameParticipantRepository.save).toHaveBeenCalledWith(
        expect.objectContaining({ status: ParticipationStatus.APPROVED }),
      );
    });
    ...
    it('should create a new participant if not existing and user/game found', async () => {
      ...
      const newParticipant = {
        game: mockGame,
        user: mockUserParticipant,
        status: setStatusDto.newStatus,
        id: 'new-gp-id',
      };
      mockGameParticipantRepository.create.mockReturnValue(newParticipant);
      mockGameParticipantRepository.save.mockResolvedValue(
        newParticipant as GameParticipant,
      );
    ...
    it('should throw NotFoundException if participant not found and game not found', async () => {
      ...
      mockUsersService.findById.mockResolvedValue(mockUserParticipant);
      await expect(service.setStatus(setStatusDto)).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('leaveGame', () => {
    it('should allow a user to leave a game successfully', async () => {
      ...
      mockGameParticipantRepository.delete.mockResolvedValue({
        affected: 1,
      } as any);

      await expect(
        service.leaveGame(mockGame.gameId, mockUserParticipant),
      ).resolves.toBeUndefined();
      expect(mockGameParticipantRepository.delete).toHaveBeenCalledWith(
        mockGameParticipant.id,
      );
    });
    ...
    it('should throw NotFoundException if game does not exist', async () => {
      mockGameRepository.findOne.mockResolvedValue(null);
      await expect(
        service.leaveGame('nonexistent-game-id', mockUserParticipant),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw ConflictException if game creator tries to leave', async () => {
      ...
      await expect(
        service.leaveGame(mockGame.gameId, mockUserCreator),
      ).rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('inviteFriendToGame', () => {
    it('should invite a friend with PENDING status if inviter is not creator', async () => {
      ...
      const invitedParticipation = {
        ...mockGameParticipant,
        user: mockUserInvited,
        status: ParticipationStatus.PENDING,
      };
      mockGameParticipantRepository.findOne.mockResolvedValue(null); // Invited user not already in game
      mockGameParticipantRepository.create.mockReturnValue(
      ...
      const result = await service.inviteFriendToGame(
        mockGame.gameId,
        mockUserParticipant,
        mockUserInvited,
      );
      ...
      expect(mockGameParticipantRepository.save).toHaveBeenCalledWith(
        expect.objectContaining({ status: ParticipationStatus.PENDING }),
      );
    });
    ...
    it('should propagate ConflictException from joinGame if invited user is already in game', async () => {
      ...
      mockGameParticipantRepository.findOne.mockResolvedValue({
        ...mockGameParticipant,
        user: mockUserInvited,
      }); // Invited user already participating
