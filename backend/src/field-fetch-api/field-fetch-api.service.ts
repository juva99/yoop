...
export class FieldFetchApiService {
  ...
  async getFields(): Promise<Field[]> {
    ...
    records.forEach((record) => {
      if (
        openingHours.some(
          (openingHours) => openingHours === record['פנוי לפעילות'],
        )
      ) {
        let cityKey;
        if (record['ישוב']) cityKey = record['ישוב']?.trim();
        else cityKey = record['רשות מקומית']?.trim();
        if (
          record['ציר X'] &&
          record['ציר X'] &&
          !Number.isNaN(Number(record['ציר X'])) &&
          !Number.isNaN(Number(record['ציר Y']))
        ) {
          [lat, lon] = this.convertXYService.convertXYtoLatLon(
            parseFloat(record['ציר X']),
            parseFloat(record['ציר Y']),
          );
  ...
  async getCities(): Promise<any> {
    ...
    const result = Array.from(yishuvMap.entries()).map(
      ([yishuv, authority]) => ({
        yishuv,
        authority,
      }),
    ...
    for (const record of records) {
      const yishuv = record['ישוב'];
      const authority = record['רשות מקומית'];
      if (yishuv) uniqueValues.add(yishuv);
      else uniqueValues.add(authority);
    }
    return [...uniqueValues];
  }
